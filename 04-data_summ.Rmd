...
```
Verify the number of group levels in `attr(*, "vars")`
```{r}
str(d.df)
```
#### Counting
Group and provide additional column n, which counts the number of rows within the group.  
```{r}
d.df = my.df %>%
  count(com, dept, prd)
...
#   summarize(count = n())
```
Counting reduce grouping by one level.  
```{r}
### result in grouping of (com, dept), 'prd' removed
str(d.df)
```
