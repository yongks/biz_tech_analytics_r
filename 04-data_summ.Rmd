...
```{r results='hold'}
my.df %>% group_by(com, dept, prd) %>% summarise_at(vars(value2:value3),mean) # col value2, value3
```
Pick **range** of variables by **column index range**
```{r results='hold'}
my.df %>% group_by(com, dept, prd) %>% summarise_at(vars(7:8),mean) # col value2, value3
```
**Multiple Functions to Multiple Columns**
```{r results='hold'}
my.df %>% group_by(com, dept, prd) %>% summarise_at(vars('value1','value2'),funs(mean,sd), na.rm=T)
```
...
- Notice after grouping (com, dept), 'prd' has been removed from grouping
```{r,results='hold' }
my.df %>% group_by(com,dept,grp) %>% attr('vars') # before summarize()
my.df %>% group_by(com,dept,grp) %>% summarize(n()) %>% attr('vars') # first summarize()
my.df %>% group_by(com,dept,grp) %>% summarize(n()) %>% summarize(n()) %>% attr('vars') # second summarize()
```
