...
Notice that `D1` is the **reference level**.  

```{r, results = 'hold'}
as.factor(my.df$dept)
```

### Releveling Factor

**Change the reference** to 'D2'.  
**`relevel`** return a new factor with base reference changed.  

```{r, results = 'hold'}
relevel(as.factor(my.df$dept), ref='D2')
```

### Dropping Levels

We removed `D2` from the vector, but the level `D2` **still exist !**  

```{r, results = 'markup'}
x = as.factor(my.df$dept)
x = x[my.df$dept!='D2']
str(x)
```

After remove unused level, notice all value will be **renumbered**.  
...
Question, can this be combined back to original factor ?
...
**`sample.split`** takes a vector (or a column from a data.frame), randomly assign TRUE (training data) and FALSE (test data) according to a specified ratio.  It returns a **logical vector for data subsetting** purpose.

> **`sample.split (Y, SplitRatio = 2/3, group = NULL)`**  
> `$\quad$ Y = vector`  
> `$\quad$ SplitRatio = ratio of split`  

Note **`sample.split`** doesn't take a data.frame, it takes a vector (or a data.frame column).

#### Create Logical Split

Load the library, check the number of rows in this dataset.  

```{r}
my.df
```

Randomly split the data and verify the **logical vector** output.  
