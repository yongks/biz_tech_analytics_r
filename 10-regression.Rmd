...
- In building model with training data, always true : 0 <= $R^2$ <= 1  
- However, if the model is **underfit**, test data (out of sample) will reveals that 0 => $R^2$ <= 1. Hence SSE is larger than SST, causing SSE/SST >= 1, therefore $R^2$ goes negative.  



### Feature Selection

1. High correlation of response and independent variable doesn't means the coefficient is statistically significant (p-value < 0.05)
2. Correlation direction (positive/negative) of response and independent variable doesn't means the coefficient will have the same direction  
...
Build a linear regresson model based on this formula: $h(x_1,x_2,x_3) = \theta_0+ \theta_1x_1 + \theta_2x_2 + \theta_3X_3$  

```{r}
fit = lm(formula = y ~ x, data = my.df)
```

#### Evaluate The Model

Check the detail of the model built, see the coef, t-value, $R^2$ and $RMSE$  

What does **`summary`** tells you ?

```{r}

```

**Extracting** More Information From The Model
