...
The dataset will be split into training set (70%), and test set (30%).  

```{r collapse=T, results='markup'}
### Load The Data
set.seed(1234)
train = read.csv('./datasets/hr.csv')
train.rows = sample( 1:nrow(train), 0.7 * nrow(train) )
train = train[train.rows,]
test = train[-train.rows,]
```
...
pred.logit = predict(fit.logit, type = 'response', newdata = test)

# alternative with specifying link function
...
- Support **many data types**: continuous, categorical, ordinal and binary variable  Yes
...
#### Information Gain (basedo on Entropy value)
...
> $\quad$ `formula  : example y ~ x1 + x2 + x3`  
> $\quad$ `data     : binomial, gaussian, poisson, quasi etc`  
> $\quad$ `method   : splitting method - 'class' for classification`  
> $\quad$ `control  : (optional) parameter used for controlling tree growth, default as following`  
...
> `              maxcompete = 4, maxsurrogate = 5, usesurrogate = 2, xval = 10,surrogatestyle = 0, maxdepth = 30)`  

The default `rpart.control` parameters specifies belowcriteria to be met before splitting a node:  
...
```{r}
fit.dtree <- rpart(left ~ ., method = "class", control = rpart.control(cp = 0.001), data = train)

dtree.pred.train = predict(fit.dtree, newdata= train, type = "class")
eval.binclass(score=dtree.pred.train, label=train$left)
```
...
dtree.pred.train.pruned = predict(fit.dtree.pruned, data=train, type="class")
eval.binclass(score=dtree.pred.train.pruned, label=train$left)[2:5]

# pruned model with test set
dtree.pred.test2 = predict(fit.dtree.pruned, newdata=test, type="class")
...
Random forest model contain multiple decision trees

#### How Does It Work ?

- Divides training data into multiple random subsets  
- Create a tree for each subsets of random data, resulting multiple trees (henced called Forest)  
- Each tree contains splitting of random variables (not all variables are being used in each tree)  
...
- No need for cross-validation or a separate test set to get an unbiased estimate of the test set error. It is estimated internally, during the run, as follows:
...
library(randomForest)
set.seed(1234)
fit.forest <- randomForest(left~., data=train, na.action=na.roughfix,importance=TRUE)
fit.forest
...
eval.binclass(score=pred.logit, label=test$left, cutoff=0.5)
```
