...
```{r, eval=FALSE}
library(rpart)
fit.dtree <- rpart(left ~ ., data=train, method="class", parms=list(split="information"))
print(fit.dtree)
summary(fit.dtree)
fit.dtree$cptable
plotcp(fit.dtree)
dtree.pruned <- prune(fit.dtree, cp=.0125)
```
...
dtree.perf <- table(train$class, dtree.pred, dnn=c("Actual", "Predicted"))
dtree.perf
```
